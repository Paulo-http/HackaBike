...
class JSSAlertView: UIViewController, UIGestureRecognizerDelegate {
    ...
    var defaultColor = UIColorFromHex(0xD0CECE, alpha: 1.0)
    
    enum TextColorTheme {
        case Dark, Light
    }
    var darkTextColor = UIColorFromHex(0x5E5E5E, alpha: 0.9)
    var lightTextColor = UIColorFromHex(0xffffff, alpha: 0.9)
    
    enum ActionType {
    case Close, Cancel, Dismiss
    }
...
//        }
    ...
    func recolorButton(color: UIColor) {
        buttonLabel.textColor = color
        if cancelButtonLabel != nil {
            cancelButtonLabel.textColor = color
        }
    ...
    func setButtonTheme(theme: TextColorTheme) {
        switch theme {
        case .Light:
            recolorButton(lightTextColor)
        case .Dark:
            recolorButton(darkTextColor)
        }
    }
    ...
    func show(viewController: UIViewController, title: String, text: String?=nil, buttonText: String?=nil, cancelButtonText: String?=nil, color: UIColor?=nil, iconImage: UIImage?=nil) -> JSSAlertViewResponder {
        ...
        self.view.backgroundColor = UIColorFromHex(0x000000, alpha: 0.0)
        ...
        bgRamon = UIView()
        bgRamon.backgroundColor = UIColorFromHex(0x000000, alpha: 0.7)
        bgRamon.frame.size = sz
        
        self.tapGesture = UITapGestureRecognizer(target: self, action: "handleTap:")
        tapGesture.delegate = self
        self.view.addGestureRecognizer(tapGesture)
        
        self.view.addSubview(bgRamon)
        ...
        alertBackgroundView.backgroundColor = defaultColor
        ...
        let buttonColor = UIImage.withColor(adjustBrightness(baseColor!, amount: 1.0))
...
//            cancelButtonLabel.alpha = 0.7
            cancelButtonLabel.textColor = defaultColor
    ...
    func handleTap(sender: UITapGestureRecognizer) {
        print("handletap:")
        self.bgRamon.removeGestureRecognizer(tapGesture)
    }
    
    func gestureRecognizer(gestureRecognizer: UIGestureRecognizer, shouldReceiveTouch touch: UITouch) -> Bool {
        if self.bgRamon.superview != nil {
            if touch.view!.isDescendantOfView(self.bgRamon) {
                closeView(true, source: .Dismiss)
                return false
            }
        }
        return true
